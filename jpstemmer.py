...
class JPStemmer:
    ...
    def step7(self):
        ...
        if self.wordg[1] == "pass":
            if self.wordg[0] == "one":
                ...
                # Check if the character exists in wordlist before trying to use its index
                if last_char in self.wordlist:
                    self.word = self.word[:-1] + self.ulist[self.wordlist.index(last_char)]
                else:
                    print(f"Character '{last_char}' not in wordlist. Skipping conjugation.")
                    return
    ...
    def step8(self):
        # This function works the same as most of the other steps, using the index of a character in a list to change it 
        # with a character with the same index in a different list.
        if self.wordg[1] == "cause":
            if self.wordg[0] == "one":
                last_char = self.word[-1]
                # Check if the last character is in the wordlist before accessing its index
                if last_char in self.wordlist:
                    self.word = self.word[:-1] + self.ulist[self.wordlist.index(last_char)]
                else:
                    print(f"Character '{last_char}' not in wordlist. Skipping causative conjugation.")
                    return
